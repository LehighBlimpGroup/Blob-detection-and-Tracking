import time
import sensor
from machine import UART, I2C, Pin, LED
...
class Tracker:
    def __init__(
        ...
        self.r_LED = LED("LED_RED")  # The red LED
        self.g_LED = LED("LED_GREEN")  # The green LED
        self.b_LED = LED("LED_BLUE")  # The blue LED
...
def setup_ov5640(advanced=False, mode=0):
    """ saturation setup NOTE: openmv's sensor.set_saturation is bugged on ov5640 """
    sensor.__write_reg(0x5580, 0x06)  # openmv default 0x06
                                      # Bit[7]: Fixed Y enable
                                      # Bit[6]: Negative enable
                                      # Bit[5]: Gray enable
                                      # Bit[4]: Fixed V enable
                                      # Bit[3]: Fixed U enable
                                      # Bit[2]: Contrast enable
                                      # Bit[1]: Saturation enable
                                      # Bit[0]: Hue enable
    if advanced:
        """ ISP setup """
        sensor.__write_reg(0x5000, 0b10100111)  # openmv default 0b00100111
        ...
        sensor.__write_reg(0x5001, 0b10100111)  # openmv default 0b10100011
                                                # Bit[7]: Special Digital Effects (SDE) enable
                                                # Bit[5]: Scaling enable
                                                # Bit[2]: UV average enable
                                                # Bit[1]: Color matrix enable
                                                # Bit[0]: Auto white balance (AWB) enable
        sensor.__write_reg(0x5005, 0b00110110)  # openmv default 0b00110110
                                                # Bit[6]: AWB bias manual enable
                                                # Bit[5]: AWB bias on enable
                                                # Bit[4]: AWB bias plus enable
                                                # Bit[2]: LENC bias on enable
                                                # Bit[1]: GMA bias on enable
                                                # Bit[0]: LENC bias manual enable
        """ color correction setup """
        sensor.__write_reg(0x5841, 0b00001101)  # openmv default 0b00001101
                                                # Bit[3]: Add BLC enable
                                                # Bit[2]: BLC enable
                                                # Bit[1]: Gain manual enable
                                                # Bit[0]: Auto Q enable
        sensor.__write_reg(0x5842, 0x00)  # openmv default 0x01 BR-H [10:8]
        sensor.__write_reg(0x5843, 0xa0)  # openmv default 0x2b BR-H [7:0]
        sensor.__write_reg(0x5844, 0x00)  # openmv default 0x01 BR-V [10:8]
        sensor.__write_reg(0x5845, 0x10)  # openmv default 0x92 BR-V [7:0]
        sensor.__write_reg(0x5846, 0x00)  # openmv default 0x01 G-H [10:8]
        sensor.__write_reg(0x5847, 0x80)  # openmv default 0x8f G-H [7:0]
        sensor.__write_reg(0x5848, 0x00)  # openmv default 0x01 G-V [10:8]
        sensor.__write_reg(0x5849, 0xa0)  # openmv default 0x0c G-V [7:0]

    if mode == 0:
        sensor.__write_reg(0x5583, 0x88)
        sensor.__write_reg(0x5584, 0x00)
    else:
        pass
...
def init_sensor_target(tracking_type:int, framesize=sensor.HQVGA, windowsize=None) -> None:
    ...
    if tracking_type == 1:
        ...
        set_fov_ov5640(width=320, height=240, sensor_portion=0.7)
        ...
        sensor.set_auto_exposure(True, exposure_us=exposure_time_us)
        sensor.set_auto_gain(True, gain_db=global_gain)
...
#    detectors = [purpleDet, greenDet]
    ...
    # Initialize UART
    uart = UART(1, baudrate= 115200, timeout_char=10) # (TX, RX) = (P4, P5)
