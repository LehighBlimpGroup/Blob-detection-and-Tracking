...
# tracking parameters
MAX_LOST_FRAME_TARGET = 30 # maximum number of frames without a positive balloon detection
...
class ColorDetector:
    ...
    def distance_mahalanobis(self, stats):
        # Compute the Mahalanobis distance between a set of points and a distribution.
        stats_mean_ab = stats[1:3]
        diff_mean_ab = stats_mean_ab - self.mu_ndarray
        # Compute the Mahalanobis distance in batch
        mahalanobis_sq = np.dot(np.dot(diff_mean_ab, self.sigma_inv_ndarray), diff_mean_ab)
        return mahalanobis_sq
...
class ShapeDetector:
    def __init__(self, gridsize):
        ...
        # self.img_solid = self.create_solid(gridsize)
        # self.tot_solid = 5#sum(self.img_square.get_pixel(j, i) for i in range(self.gridsize) for j in range(self.gridsize))
    ...
    def create_solid(self, gridsize):
        img = sensor.alloc_extra_fb(gridsize, gridsize, sensor.BINARY)
        radius = (gridsize)// 2
        img.draw_circle(gridsize // 2, gridsize // 2, radius, color=255, fill=True, thickness=1)
        if (gridsize % 2 == 0):
            img.draw_circle((gridsize) // 2 -1, (gridsize) // 2 -1, radius, color=255, fill=True)
            img.draw_circle((gridsize) // 2 , (gridsize) // 2 -1, radius, color=255, fill=True)
            img.draw_circle((gridsize) // 2 -1, (gridsize) // 2 , radius, color=255, fill=True)
        return img
    ...
    def downsample_and_average(self, roi_img):
        ...
        for i in range(self.gridsize):
            for j in range(self.gridsize):
                ...
                mean_val = stats.mean()
                binary_val = 1 if mean_val > 65 else 0  # Threshold the mean to create a binary image
    ...
    def detect_shape(self, roi_img):
            ...
            for j in range(self.gridsize):
                if mean_pooled_img.get_pixel(j, i) == 1:  # Check if the ROI pixel is white
                    ...
                    if self.img_square.get_pixel(j, i) == 1:
                        overlap_square += 1
                    # if self.img_solid.get_pixel(j, i) == 1:
                    #     overlap_solid += 1
        ...
        # if overlap_solid/self.tot_solid > overlap_triangle/self.tot_tri and overlap_solid/self.tot_solid > overlap_circle/self.tot_cir and overlap_solid/self.tot_solid > overlap_square //self.tot_squ:
        #     return "solid"
...
class GoalTracker:
    ...
    def update_leds(self, tracking: bool = False, detecting: bool = False, lost: bool = True) -> None:
        ...
        # if TARGET_COLOR == TARGET_YELLOW:
        #     self.g_LED.on()
        #     self.r_LED.on()
        #     self.b_LED.off()
        # elif TARGET_COLOR == TARGET_ORANGE:
        #     self.g_LED.on()
        #     self.r_LED.off()
        #     self.b_LED.on()
        if tracking and detecting and not lost:
            ...
            pass
        # if TARGET_COLOR == TARGET_ORANGE:
        #     self.b_LED.on()
        #     self.r_LED.off()
        # elif TARGET_COLOR == TARGET_YELLOW:
        #     self.r_LED.on()
        #     self.b_LED.off()
    ...
    def detect(self, isColored=False, edge_removal=True):
        ...
        # if self.num_blob_hist > 2 and self.thresholds == TARGET_ORANGE:
        # if self.thresholds == TARGET_ORANGE:
        ...
        for blob in list_of_blob:
            if blob.area() > 10 and line_length(blob.minor_axis_line())> 3:
                ...
                if roi_img:
                    ...
                    if detected_shape != "not" and detected_shape != "solid":
                        big_blobs.append(blob)
...
def detector_initialization(mode):
    ...
    for color in color_collection:
        ...
        elif color in COLOR_DEMONS:
            detector_type = "T"
            rgb = COLOR_DEMONS[color]
...
if __name__ == "__main__":
    ...
    if mode == 0 or mode == 3:
        detectors = detector_initialization(mode)
    else:
        detectors = None
    solid_red_detector = ColorDetector(
        "neutral_red", line_ref = None,
        detector_type="P",
    ...
    while True:
        ...
        if uart.any():
            ...
            # print(uart_input)
            if uart_input[-1] == 0x40 and mode != 0:
                if mode == 1 or mode == 2:
                    # yellow or orange goal modes
                    tracker.destruct()
                led_blue.on()
                led_green.on()
                # initialize normal balloon color detectors
                detectors = detector_initialization(0)
                IBus_message([0, 0, 0, 0, 0, 0, 0, 0, 0, 9999])
                res = mode_initialization(0, mode, grid, detectors)
                if res:
                    mode, tracker = res
            elif uart_input[-1] == 0x80 and mode != 1:
                TARGET_COLOR = TARGET_YELLOW
                led_red.on()
                led_green.on()
                if mode == 2:
                ...
                if res:
                    mode, tracker = res
            elif uart_input[-1] == 0x81 and mode != 2:
                TARGET_COLOR = TARGET_ORANGE
                led_red.on()
                if mode == 1:
                    tracker.destruct()
                IBus_message([0, 0, 0, 0, 0, 0, 0, 0, 0, 9999])
                res = mode_initialization(2, mode)
                if res:
                    mode, tracker = res
            elif uart_input[-1] == 0x41 and mode != 3:
                if mode == 1 or mode == 2:
                    # yellow or orange goal modes
                    tracker.destruct()
                led_blue.on()
                # initialize demon slayer color detectors
                detectors = detector_initialization(3)
                IBus_message([0, 0, 0, 0, 0, 0, 0, 0, 0, 9999])
                res = mode_initialization(3, mode, grid, detectors)
                if res:
                    mode, tracker = res
            else:
